 ...
 * 这里依然要用到Cookies
 *
 * 根据 https://github.com/yaphone/itchat4j/blob/master/src/main/java/cn/zhouyafeng/itchat4j/utils/enums/RetCodeEnujava
 * retcode：
 * 	NORMAL("0", "普通"),
 *	LOGIN_OUT("1102", "退出"),
...
func SyncCheck(loginMap *LoginMap) (int64, int64, error) {
	...
	urlMap[enum.Sid] = loginMap.BaseRequest.Sid
	urlMap[enum.Uin] = loginMap.BaseRequest.Uin
	urlMap[enum.DeviceId] = loginMap.BaseRequest.DeviceID
	urlMap[enum.SyncKey] = loginMap.SyncKeyStr
	urlMap[enum.TimeStamp] = urlMap[enum.R]
	...
	resp, err := client.Get(enum.SYNC_CHECK_URL + GetURLParams(urlMap))
	if err != nil {
		return 0, 0, err
	}
...
func WebWxSync(loginMap *LoginMap) (WxRecvMsges, error) {
	...
	resp, err := http.Post(enum.WEB_WX_SYNC_URL+GetURLParams(urlMap), enum.JSON_HEADER, strings.NewReader(string(jsonBytes)))
	if err != nil {
		return wxMsges, err
	}
	defer resp.Body.Close()
...
func InviteMember(loginMap *LoginMap, memberUserName string, chatRoomUserName string) error {
	...
	wxUpdateChatRoomMap[enum.BaseRequest] = loginMap.BaseRequest
	...
	_, err = client.Post(enum.WEB_WX_UPDATECHATROOM_URL+GetURLParams(urlMap), enum.JSON_HEADER, strings.NewReader(string(jsonBytes)))
